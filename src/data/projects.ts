...
export const projects: Project[] = [
  ...
  },
  {
    ...
    status: 'Concept',
    showInGrid: true,
    iconGradient: 'linear-gradient(135deg, #030A12 0%, #061E30 100%)',
    iconLabel: '◎',
    iconColor: '#00A8E0',
    problem:
      "Alexa resets to zero at the start of every conversation. No persistent memory. No household identity model. No ability to distinguish between family members reliably. A product that calls itself an 'assistant' but forgets every interaction is a contradiction. Smart home routines fail silently and users never learn why — so they abandon automation entirely. After the initial novelty wears off, engagement collapses because there is no relationship to deepen.",
    solution:
      "Persistent Memory Layer (opt-in): a user-consent-gated system where Alexa retains context across sessions — preferences, recurring patterns, ongoing tasks. Voice ID per household member with automatic profile switching. Proactive micro-context: 'You usually ask for traffic at 7:50am. Want me to start automatically?' Silent failure alerts with plain-language diagnosis. This repositions Alexa from a command tool to an ambient relationship — the missing layer between smart home hardware and genuine household utility.",
    role: 'PM (Unsolicited Analysis)',
    keyDecisions: [
      'Opt-in memory architecture preserves user trust while enabling personalisation at household scale',
      'Voice ID per member solves the shared-device personalisation problem no smart speaker has cracked',
      'Proactive micro-context shifts Alexa from reactive to anticipatory — a fundamental positioning change',
      'Plain-language failure alerts close the silent failure loop that destroys routine adoption over time',
    ],
    stack: ['Persistent Memory Layer', 'Voice ID Engine', 'Proactive Context System', 'Household Identity Model'],
    metrics: [
      { label: 'DAU retention', value: '+24%' },
      { label: 'Routine adoption', value: '+40%' },
      { label: 'Household NPS', value: '34 → 58' },
      { label: 'Type', value: 'Unsolicited analysis' },
    ],
    timeline: 'Analysis — 2025',
  },
  {
    id: 'linkedin-signal',
    name: 'LinkedIn Signal',
    tagline: "A network of 1 billion people where nobody can find the right signal.",
    status: 'Concept',
    showInGrid: true,
    iconGradient: 'linear-gradient(135deg, #010810 0%, #021525 100%)',
    iconLabel: 'in',
    iconColor: '#0A66C2',
    problem:
      "LinkedIn's feed is algorithmically optimised for engagement, not relevance. The result: corporate announcements, motivational quotes, and 'excited to announce' posts that train users to disengage. Job seekers apply to hundreds of roles and hear nothing. Hiring managers are buried in unqualified applications. The connection model conflates 'colleague' with 'contact' with 'person I met once' — destroying the signal value of a professional recommendation.",
    solution:
      "Three interventions targeting the same root cause. (1) Feed Quality Score — a user-adjustable signal/noise dial that surfaces only content from people you've actually worked with or follow for genuine professional insight. (2) Verified Skills badges — short async assessments that validate claimed skills before an application reaches a recruiter. (3) Warm Intro Layer — when applying, surface first and second-degree connections at the company with a one-tap 'ask for context'.",
    role: 'PM (Unsolicited Analysis)',
    keyDecisions: [
      'Feed Quality Score surfaces a tradeoff LinkedIn has never made visible — engagement vs. professional relevance',
      'Verified Skills shifts the application from an unverified claim to a validated signal before it reaches a recruiter',
      'Warm Intro Layer converts the passive network graph into an active job-seeking tool at zero additional cost',
      'All three interventions target the same root cause: the platform optimises for time-on-platform over value delivered',
    ],
    stack: ['Feed Quality Score', 'Verified Skills Engine', 'Warm Intro Layer', 'Network Graph API'],
    metrics: [
      { label: 'Recruiter response', value: '+35%' },
      { label: 'App-to-interview', value: '+28%' },
      { label: 'Feed NPS', value: '+22pts' },
      { label: 'Type', value: 'Unsolicited analysis' },
    ],
    timeline: 'Analysis — 2025',
  },
